...
class DatePicker extends StatefulWidget {
  ...
  /// Background color for the unselected item
  ...
  DatePicker(
    ...
    this.type = DateWidgetType.rounded_rect,
    ...
    this.locale = "en_US",
    this.events = const [],
    this.showEventsIndicators = true,
    this.showMonth = true,
    this.monthDivider,
    this.showMonthDivider = true,
  }) : assert(
...
class _DatePickerState extends State<DatePicker> {
  ...
  void initState() {
    ...
    this.selectedMonthStyle = widget.monthTextStyle.copyWith(
        color: widget.type == DateWidgetType.circle_date
            ? widget.selectionColor
            : widget.selectedTextColor);
    this.selectedDayStyle = widget.dayTextStyle.copyWith(
        color: widget.type == DateWidgetType.circle_date
            ? widget.selectionColor
            : widget.selectedTextColor);
  ...
  Widget build(BuildContext context) {
    return Container(
      height: widget.height + (((widget.width * 0.1) + 10) * 2),
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          //Create a list of events that are off this day
          List<EventModel> itemEvents = widget.events.where((element) {
            DateTime tempStartDate = new DateTime(element.startDateTime.year,
                element.startDateTime.month, element.startDateTime.day);
            DateTime tempEndDate = new DateTime(element.endDateTime.year,
                element.endDateTime.month, element.endDateTime.day);
            if (date == tempStartDate) {
              return true;
                ...
                (date.isAfter(tempStartDate) &&
                    ...
                    element.repeats != EventRepeats.never) */
                ) {
              return true;
            } else {
              return false;
            ...
            children: [
              Builder(builder: (context) {
                if (prevDate.day > date.day &&
                    (widget.showMonthDivider ?? true)) {
                  _dividerCount++;
                  return widget.monthDivider ??
                      Container(
                        padding:
                            EdgeInsets.symmetric(vertical: widget.height / 2.5),
              ...
              DateWidget(
                date: date,
                type: widget.type,
                eventIndicatorColors: [
                  ...itemEvents.map((e) => e.eventColor).toList()
                ],
                ...
                selectionColor: isSelected
                    ? widget.selectionColor ?? AppColors.defaultSelectionColor
                ...
                showEventsIndicators: widget.showEventsIndicators ?? true,
                showMonth: widget.showMonth ?? false,
              ),
            ],
          );
        },
